...
const mongosession=require("connect-mysql-session")(session)
...
app.post("/login_hospital",async (req,res)=>{
    try{
        const chk = await hospinfo.findOneBy({email:req.body.email})
        if(chk){
...
app.use('/',mainRouter);
app.use('/login_signup', loginhospitalRouter);
...
app.use('/hospitaldetails', hospdetailRouter);